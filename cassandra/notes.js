...
Web.”
consistency level is a setting that clients must specify on every operation and that allows you to decide how many replicas in the cluster must acknowledge a write op- eration or respond to a read operation in order to be considered successful.
...
***************
2 FEATURES
 2.1 distributed, no-master, gossip protocol
 2.2 elastic scalability, up and down by adding nodes, balances automatically
 2.3 highly available and fault tolerant
 2.4 tuneable consistency
    in general consistency types => strict, causal (vector clocks), eventual
    2.4.1 replication factor -> number of nodes to propagate a write action
    2.4.2 consistency factor -> number of nodes to acknowledge a write action
 ...
 2.6 data store:  internally, cassandra is more row-oriented than column-oriented, stores as multi-dimensional hashmap.
 2.7 schema free
 2.8 scales well with multi-core and multiple datacenters.
 2.9 high write throughput.
 2.10 good for multi node deployments of significant scale.
